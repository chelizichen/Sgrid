<template>
  <div>
    <el-table border :data="servantList">
      ...
      <el-table-column label="操作" align="center">
        <template #default="scoped">
          <el-button @click="updateServant(scoped.row)" type="text">修改</el-button>
          <el-button v-if="scoped.row.stat != -1" @click="deleteServant(scoped.row, -1)" type="text"
            >停用</el-button
          >
          <el-button
            v-else
            @click="deleteServant(scoped.row, 0)"
            type="text"
            style="color: var(--sgrid-primary-choose-color)"
            >启用</el-button
          >
  ...
  <el-dialog v-model="cronTaskDialogVisible" title="定时设置" width="80%">
    <el-form :model="form" label-width="auto">
      ...
      <el-form-item label="Activity Time">
        <el-date-picker v-model="form.activeTime" type="datetime" placeholder="上线时间" />
      </el-form-item>
      <el-form-item label="ExpireTime Time">
        <el-date-picker v-model="form.expireTime" type="datetime" placeholder="下线时间" />
      </el-form-item>
